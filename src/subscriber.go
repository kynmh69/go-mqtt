...
func main() {
	...
	var fn mqtt.MessageHandler = func(client mqtt.Client, msg mqtt.Message) {
		msgCh <- msg
		fmt.Println("msgCh", msgCh)
	}
	...
	if subscribeToken := cc.Subscribe("go-mqtt/sample", 0, fn); subscribeToken.Wait() && subscribeToken.Error() != nil {
